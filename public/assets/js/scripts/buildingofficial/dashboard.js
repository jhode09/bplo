...
var KtCharts = function() {
    ...
    var ownership_report = function () {
        $.ajax({
            url : ownership_report_url,
            type: "post",
            success:function(res) { 
                
                $('#rent_count').text(_commaSeparateNumber(res.rent));
                $('#owned_count').text(_commaSeparateNumber(res.owned));

                var rent = res.rent / 100;
                var owned = res.owned / 100;
                var element = document.getElementById("ownership_report");
                var height = parseInt(KTUtil.css(element, 'height'));
