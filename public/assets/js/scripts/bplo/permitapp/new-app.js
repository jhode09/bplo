...
var countBoard   = 0;
var bussID, bussLineDesc, bussClassID, bussClassTaxID;
...
// ========== BEGIN STEP 4 ========== //
    ...
    var KTAdd = function(id, desc, classID, classTaxID) {
        $("#bussCapitalAmount").val('');
        bussID         = id;
        bussLineDesc   = desc;
        bussClassID    = classID;
        bussClassTaxID = classTaxID;
        var exists = false;
    ...
    var KTEditCapital = function (index, capital) {
        lineAction = "Update";
        lineIndex  = index;
        $("#grossModal").modal('show');
    ...
    var KTDisplayBussLines = function() {
        ...
        $.each(bussLines, function(index, item){
            var output = '<span class="label label-xl label-success label-pill label-inline mr-5 mt-2 p-10">\
                            <a type="button" onclick="KTEditCapital('+index+','+item.bussCapitalAmount+')">'
                                + item.bussLineDesc + " <br> BUSINESS CAPITAL - ₱ " + AddComma(item.bussCapitalAmount) +
...
var KTCreateApplication = function () {
    ...
    // ========== BEGIN STEP 4 ========== //
        ...
        var _add = function() {
            var bussCapitalAmount = $("#bussCapitalAmount").val().replace('₱ ', '').replaceAll(',', '');
    ...
    var _initValidation = function () {
        ...
        $("#btn_submit_application").on('click', function(e) {
            ...
            bussFiles = new Array();
            $("form#form_application input[type='file']").each(function(){
                if( $(this).val() != "") {
                    var data = {
                        "req_id"   : $(this).attr("id"),
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var fd = new FormData($("#form_application")[0]);
                    fd.append('bussLines', JSON.stringify(bussLines));
                    fd.append('bussFiles', JSON.stringify(bussFiles));
                    fd.append('boardMembers', JSON.stringify(boardMembers));
                    $.ajax({
                        url : create_app_url,
                        data: fd,
                        processData: false,
            ...
            var msg = "<div class='form-group mb-5'>\
                    ...
                    <div class='form-group row p-0 m-0'>\
                        ...
                        <div class='col-7 text-left mb-3'>\
                            <span class='font-weight-bolder font-size-sm'>New Business</span>\
                        </div>\
                    </div>\
                    <div class='form-group row p-0 m-0'>\
                    ...
                    if (_wizard.getStep() == 1) {
                       ...
                       _wizard.goNext();
                        ...
                        if (bussLines.length <= 0) {
                            Swal.fire({
                                title: "Failed!",
                                text: "Please select at least one in business lines.",
                                icon: "error",
                                buttonsStyling: false,
                                confirmButtonText: "Ok, got it!",
                                customClass: {
    ...
    var _displayData = function() {
        ...
        //Buss Lines
        $.each(bussLines, function(index, item){
            bussLineOutput += '<span class="label label-xl label-success label-pill label-inline mr-2 p-10 font-weight-bolder">' + item.bussLineDesc + " <br> BUSINESS CAPITAL - ₱ " + AddComma(item.bussCapitalAmount) + '</span>';
        });
        $('#businessLines').empty();
        ...
        //Display Data
        for (var i = 0; i < $("#form_application")[0].length; i++) {
            var formType = $("#form_application")[0][i].type;
            var formVal  = $("#form_application")[0][i].value;
            var formName = $("#form_application")[0][i].name;
    ...
    return {
        ...
        init: function() {
            ...
            _formEl   = KTUtil.getById('form_application');
